...
def run(swarm, num_krauss, num_control, scenario):
    ...
    for k in range(num_krauss):
        route = np.mod(k, 8)
        traci.vehicle.add(str(k), "route_" + str(route), typeID="krauss", depart=0, departLane="first",
    ...
    for c in range(num_control):
        route = np.mod(c, 8)
        traci.vehicle.add(str(c + num_krauss), "merge_route_" + str(route), typeID="krauss_control", depart=0,
