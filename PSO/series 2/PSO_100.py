...
def pso(num_krauss, num_control, scn):
    ...
    for p in particle_location:
        particle_value, max_merges, min_collisions, m, c, ls, ms = Function(p, max_merges, min_collisions, num_krauss,
                                                                            num_control,
                                                                            scn)  # particle_location[:,0],particle_location[:,1], particle_location[:,2],
    ...
    for iteration_i in range(iterations):  # for each iteration
        ...
        for particle_i in range(swarm_size):  # for each particle
            ...
            v, max_merges, min_collisions, m, c, ls, ms = Function(particle_location[particle_i], max_merges,
                                                                   min_collisions, num_krauss, num_control,
                                                                   scn)  # particle_location[particle_i,0], particle_location[particle_i,1], particle_location[particle_i,2],
            swarm_merge.append(m)
            swarm_coll.append(c)
            swarm_ls.append(ls)
            swarm_ms.append(ms)
        ...
        swarm_ms_avg.append(statistics.mean(swarm_ms))
        swarm_fit_avg.append(statistics.mean(swarm_fitness))
        print('solution for speed=' + '%.2f' % global_best_location[0] + ', min gap=' + '%.2f' % global_best_location[
            1] + ", max merges: " + str(max_merges) + ", min collisions: " + str(min_collisions))
    ...
    with open('pso_' + str(num_krauss + num_control) + '_scenario_' + str(scn) + '_ratio_' + str(k) + "_" + str(
            c) + '.csv', mode='w') as file:
        file_writer = csv.writer(file, delimiter=',')
        file_writer.writerow(["Swarm Average Merges", "GB Merges", "Swarm Average Collisions", "GB Collisions", "Swarm Average Loop Speeds", "GB Loop Speeds", "Swarm Average Merge Speeds", "GB Merge Speeds"])
